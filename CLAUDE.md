...
**13. 서버 상태 확인 API(헬스체크)**
- 배경: 프론트엔드 초기 로드 시 백엔드 연결 여부를 확인할 방법 필요
- 과정: Spring Boot Actuator 도입 vs 경량 커스텀 컨트롤러 검토
- 결정: Actuator 미도입, 기존 컨트롤러와 동일한 스타일의 `GET /api/health` 커스텀 엔드포인트 추가
- 효과: 신규 의존성 없이 서버 연결 확인 가능. 프론트는 최초 로드 시 1회만 확인 후 확장자 목록 조회, 이후 수정/삭제/생성 시에는 재확인 없이 목록만 재조회

**14. 커스텀 확장자 이름 시작 문자 제약**
- 배경: 숫자로 시작하는 확장자는 실존하지 않고 실행되지도 않음
- 과정: 기존 정규식("숫자만 불가")은 "1abc"처럼 숫자로 시작하되 문자가 섞인 값은 허용하는 허점이 있었음
- 결정: 정규식을 `^[A-Za-z][A-Za-z0-9]*$`로 변경해 첫 글자를 영문으로 강제(기존 "숫자만 불가" 조건은 자동으로 포함됨)
- 효과: 실제 존재/실행 가능한 확장자 형태만 등록되도록 검증 강화

**15. CORS Preflight 캐시**
- 배경: 무료 티어 + 원거리 리전(미국 동부) 배포로 매 요청마다 OPTIONS preflight가 반복되어 지연이 체감됨
- 과정: PATCH/DELETE는 CORS 스펙상 preflight 생략이 불가능함을 확인, 캐싱 여부만 개선 가능한 부분으로 판단
- 결정: `WebConfig`의 CORS 설정에 `.maxAge(3600)` 추가
- 효과: 동일 origin+메서드+URL 조합은 1시간 내 재요청 시 preflight 생략. id별로 URL이 달라지는 요청(체크 토글, 커스텀 삭제)은 각 id당 최초 1회는 여전히 발생
